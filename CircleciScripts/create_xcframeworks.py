...
MACOS_DEVICE_ARCHIVE_PATH = f"{PWD}/xcframeworks/output/macOS/"
XCFRAMEWORK_PATH = f"{PWD}/xcframeworks/output/XCF/"

def create_archive(framework, project_file):
    archive_path = f"{MACOS_DEVICE_ARCHIVE_PATH}{framework}"
    ...
    if exit_code == 0:
        log(f"Created macOS archive {framework} {destination}")
    else:
...
def map_framework_to_project(framework_list):
    ...
    for framework in framework_list:
        if framework not in str(out):
            ...
            framework_map[framework] = "AWSmacOSSDKv2.xcodeproj"
...
def create_xc_framework(framework):
    ios_device_framework = f"{MACOS_DEVICE_ARCHIVE_PATH}{framework}.xcarchive/Products/Library/Frameworks/{framework}.framework"
    ios_device_debug_symbols = f"{MACOS_DEVICE_ARCHIVE_PATH}{framework}.xcarchive/dSYMs/{framework}.framework.dSYM"
    xcframework = f"{XCFRAMEWORK_PATH}{framework}.xcframework"
    if os.path.exists(xcframework):
        log(f"skipping {framework}...")
    else:
